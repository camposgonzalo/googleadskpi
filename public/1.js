...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    dailyCost: function dailyCost() {
      var t_camp = this.form.type == "Libre" ? 1.1 : 0.9;
      var igv = this.form.apply_billing ? 1.8 : 1.0;
      this.form.monthly_cost = this.active_day_constant * this.checkList.length * this.form.daily_cost;
      this.form.total_cost = this.buho_comision_constant * this.form.monthly_cost * t_camp * igv;
    },
    monthlyCost: function monthlyCost() {
      var t_camp = this.form.type == "Libre" ? 1.1 : 0.9;
      var igv = this.form.apply_billing ? 1.8 : 1.0;
      this.form.daily_cost = this.form.monthly_cost / (this.active_day_constant * this.checkList.length);
      this.form.total_cost = this.buho_comision_constant * this.form.monthly_cost * t_camp * igv;
    },
    totalCost: function totalCost() {
      var t_camp = this.form.type == "Libre" ? 1.1 : 0.9;
      var igv = this.form.apply_billing ? 1.8 : 1.0;
      this.form.monthly_cost = this.form.total_cost / (this.buho_comision_constant * t_camp * igv);
      this.form.daily_cost = this.form.monthly_cost / (this.active_day_constant * this.checkList.length);
    },
    updateCost: function updateCost() {
      if (this.form.daily_cost != 0) this.dailyCost();else if (this.form.monthly_cost != 0) this.monthlyCost();else this.totalCost();
    }
  }
});
